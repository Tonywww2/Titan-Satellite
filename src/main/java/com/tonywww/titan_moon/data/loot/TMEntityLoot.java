 ...
 */
public class TMEntityLoot implements LootTableSubProvider {

    //? if neoforge {
    /*private final HolderLookup.Provider provider;

    public TMEntityLoot(HolderLookup.Provider provider) {
    ...
    *///?}
        consumer.accept(entity("cryo_scavenger"), LootTable.lootTable()
                .withPool(singleDrop(TMItems.CRYO_CARAPACE.get(), 0, 2)));

        consumer.accept(entity("ammonia_stalker"), LootTable.lootTable()
                .withPool(singleDrop(TMItems.TOXIC_GLAND.get(), 0, 2)));

        consumer.accept(entity("corrupted_probe"), LootTable.lootTable()
                .withPool(singleDrop(TMItems.DEPLETED_BATTERY.get(), 0, 1))
                .withPool(singleDrop(TMItems.PRECISION_COMPONENTS.get(), 0, 2)));

        consumer.accept(entity("tholin_weaver"), LootTable.lootTable()
                .withPool(singleDrop(TMItems.TOUGH_NEURAL_GLAND.get(), 0, 1))
                .withPool(singleDrop(TMItems.THOLIN_SILK_SAC.get(), 0, 2)));

        consumer.accept(entity("native_ice_worm"), LootTable.lootTable()
                .withPool(singleDrop(TMItems.TOUGH_NEURAL_GLAND.get(), 1, 2))
                .withPool(singleDrop(TMItems.CRYO_CARAPACE.get(), 0, 2)));
        ...
        consumer.accept(entity("hydrotroph_grazer"), LootTable.lootTable()
                .withPool(singleDrop(TMItems.AERO_MEMBRANE.get(), 0, 1)));
    }
    ...
    private static ResourceLocation entity(String name) {
        return TitanMoon.rl("entities/" + name);
    }
    //?} else {
    /*private static ResourceKey<LootTable> entity(String name) {
        return ResourceKey.create(Registries.LOOT_TABLE, TitanMoon.rl("entities/" + name));
    }
    *///?}
}
