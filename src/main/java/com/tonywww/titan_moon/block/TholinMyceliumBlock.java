...
public class TholinMyceliumBlock extends Block {
    ...
    private static boolean isResidue(ItemStack stack) {
        return stack.is(TMItems.CRYO_CARAPACE.get())
                || stack.is(TMItems.TOXIC_GLAND.get())
                || stack.is(TMItems.TOUGH_NEURAL_GLAND.get())
                || stack.is(TMItems.THOLIN_SILK_SAC.get())
                || stack.is(TMItems.AERO_MEMBRANE.get())
                || stack.is(TMItems.THOLIN_FIBRE.get());
    }
}
