 ...
 * Properties 在方法里延迟构建（在注册阶段调用），避免与 {@link TMBlocks}/{@link TMItems}
 * 产生类初始化期的循环引用。
 */
public final class TMFluids {

    private TMFluids() {
    }

    public static final DeferredRegister<Fluid> REGISTER =
            DeferredRegister.create(Registries.FLUID, TitanMoon.MODID);
    ...
    private static ForgeFlowingFluid.Properties methaneProperties() {
        return new ForgeFlowingFluid.Properties(TMFluidTypes.LIQUID_METHANE, LIQUID_METHANE, FLOWING_LIQUID_METHANE)
                .slopeFindDistance(3)
                .levelDecreasePerBlock(2)
                .block(TMBlocks.LIQUID_METHANE_BLOCK)
                .bucket(TMItems.LIQUID_METHANE_BUCKET);
    }

    private static ForgeFlowingFluid.Properties ammoniaProperties() {
        return new ForgeFlowingFluid.Properties(TMFluidTypes.LIQUID_AMMONIA, LIQUID_AMMONIA, FLOWING_LIQUID_AMMONIA)
                .slopeFindDistance(2)
                .levelDecreasePerBlock(1)
                .block(TMBlocks.LIQUID_AMMONIA_BLOCK)
                .bucket(TMItems.LIQUID_AMMONIA_BUCKET);
    }

    private static ForgeFlowingFluid.Properties hydrogenProperties() {
        return new ForgeFlowingFluid.Properties(TMFluidTypes.LIQUID_HYDROGEN, LIQUID_HYDROGEN, FLOWING_LIQUID_HYDROGEN)
                .slopeFindDistance(2)
                .levelDecreasePerBlock(1)
                .block(TMBlocks.LIQUID_HYDROGEN_BLOCK)
                .bucket(TMItems.LIQUID_HYDROGEN_BUCKET);
    }
    //?} else {
    /*public static final Supplier<FlowingFluid> LIQUID_METHANE =
    ...
    private static BaseFlowingFluid.Properties methaneProperties() {
        return new BaseFlowingFluid.Properties(TMFluidTypes.LIQUID_METHANE, LIQUID_METHANE, FLOWING_LIQUID_METHANE)
                .slopeFindDistance(3)
                .levelDecreasePerBlock(2)
                .block(TMBlocks.LIQUID_METHANE_BLOCK)
                .bucket(TMItems.LIQUID_METHANE_BUCKET);
    ...
    private static BaseFlowingFluid.Properties ammoniaProperties() {
        return new BaseFlowingFluid.Properties(TMFluidTypes.LIQUID_AMMONIA, LIQUID_AMMONIA, FLOWING_LIQUID_AMMONIA)
                .slopeFindDistance(2)
                .levelDecreasePerBlock(1)
                .block(TMBlocks.LIQUID_AMMONIA_BLOCK)
                .bucket(TMItems.LIQUID_AMMONIA_BUCKET);
    ...
    private static BaseFlowingFluid.Properties hydrogenProperties() {
        return new BaseFlowingFluid.Properties(TMFluidTypes.LIQUID_HYDROGEN, LIQUID_HYDROGEN, FLOWING_LIQUID_HYDROGEN)
                .slopeFindDistance(2)
                .levelDecreasePerBlock(1)
                .block(TMBlocks.LIQUID_HYDROGEN_BLOCK)
                .bucket(TMItems.LIQUID_HYDROGEN_BUCKET);
