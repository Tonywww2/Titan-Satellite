...
public final class TitanFluidInteractions {
    ...
    private static void registerInteractions() {
        ...
        addSolidify(TMFluidTypes.LIQUID_METHANE.get(), TMFluidTypes.LIQUID_AMMONIA.get(),
                TMBlocks.PACKED_METHANE_ICE.get().defaultBlockState(),
                TMBlocks.CRUSHED_ICE.get().defaultBlockState());
        // 液态氨遇液态甲烷 → 氨侧速冻（源=低温冰，流动=碎冰）
        addSolidify(TMFluidTypes.LIQUID_AMMONIA.get(), TMFluidTypes.LIQUID_METHANE.get(),
                TMBlocks.CRYO_ICE.get().defaultBlockState(),
                TMBlocks.CRUSHED_ICE.get().defaultBlockState());
    }
