...
public final class TSEntities {
    ...
    // 原生冰虫（敌对·精英，巢穴守卫）。
    public static final RegistryObject<EntityType<NativeIceWorm>> NATIVE_ICE_WORM = REGISTER.register("native_ice_worm",
            () -> EntityType.Builder.of(NativeIceWorm::new, MobCategory.MONSTER)
                    .sized(1.2F, 1.0F).clientTrackingRange(10).build("native_ice_worm"));
    // 甲烷微浮群（被动·飞行群集，浮游体食源）。
    public static final RegistryObject<EntityType<MethaneMidge>> METHANE_MIDGE = REGISTER.register("methane_midge",
            () -> EntityType.Builder.of(MethaneMidge::new, MobCategory.CREATURE)
                    .sized(0.3F, 0.3F).clientTrackingRange(6).build("methane_midge"));
    // 氢营养蹒兽（被动·食草，充实营养级）。
    public static final RegistryObject<EntityType<HydrotrophGrazer>> HYDROTROPH_GRAZER = REGISTER.register("hydrotroph_grazer",
