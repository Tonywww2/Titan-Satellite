# 土卫六 (Titan Moon) — 平行任务表 (Parallel Task Plan · multi-agent)
...
### 2.1 包结构 / 目录
```
com.tonywww.titan_moon/
  TitanMoon.java        # 主类：唯一 wiring 点（Stage A 冻结）
...
assets/titan_moon/  blockstates/ models/ textures/ lang/ sounds/
```

### 2.2 命名 / ID（全部 snake_case，命名空间 `titan_moon`）
...
- **群系标签**：`titan_moon:is_titan`（列出 5 群系，供生成注入/特效判定）。
- **自定义 density function**：`titan_moon:biome_height`（+ 组合用 `floor/ceiling/final` 视需要）。
...
- **特征注入**（CR-4 变更）：biome JSON 的 `features` 一律**留空**；特征由 PB-3 用 `forge:add_features` biome_modifier（`data/.../forge/biome_modifier/*.json`，targeting `#titan_moon:is_titan`，指定 step）注入。与刷怪同模式，PB-1↔PB-3 完全解耦、biome 不引用特征、避免缺失特征崩溃。
...
- Owns: `registry/TS*.java`（含新增 `TSMobEffects`、`TSParticles`、`TSBlockEntities`）、`TitanMoon.java`、`block/*Block.java`（桩）、`entity/*.java`（桩，含 `probe_laser`）、`item/*.java`（桩）、`client/*Renderer.java`（桩）+`client/TitanClientEvents.java`、`lang/en_us.json`+`lang/zh_cn.json`。
- Reads: §2 全部 ID/签名。
...
- Owns: `worldgen/`（`TSWorldgenTypes` 注册 DensityFunction/Feature/Structure codec + `density/`,`feature/`,`structure/` 桁类）、`event/MethaneExtractionWaveEvent.java`、`mixin/`（`titan_moon.mixins.json` + Mixin 插件 + `package-info`）、`data/.../tags/worldgen/biome/is_titan.json`。（~~`capability/*`~~、~~`data/.../damage_type/*`~~ 已随 **CR-5** 移除）
...
> **Gate B**：`runServer`→`execute in titan_moon:titan` 探测：5 群系、高差地形、按群系地表、特征生成。
...
**PG-1 · 新表层块 + 群系显示名（前置：Gate B）**  ☑ owner:agent1 output:新增 3 块——weathered_titan_stone(TERRACOTTA_GRAY、复用 titan_stone 贴图)/sedimentary_titan_stone(TERRACOTTA_BROWN、复用 titan_basalt 贴图)=cube_all 表层块；branch_crystal(COLOR_LIGHT_BLUE、noCollission+noOcclusion+lightLevel4)=cross 十字装饰(复用原版 warped_roots 形态 + model render_type cutout)。TSBlocks/TSItems(BlockItem)/TSCreativeTabs(创造栏) 均新增不改现有(CR-8)。9 assets(blockstates×3+models/block×3+models/item×3)。lang 中英加 3 新块名 + **六群系“土卫六·”显示名**(biome.titan_moon.* 含新 barren_plateau)。build --rerun-tasks BUILD SUCCESSFUL；runClient 到标题页资源加载无 titan_moon model/texture/lang 缺失警告(blocks atlas 拼合成功、mod 加载)。**冻结 3 新块 id 供 PG-2/4 只读引用。**
...
| 文件 / 目录 | 主人 | 阶段 |
|---|---|---|
| `registry/TS*.java`, `TitanMoon.java`, `lang/*.json`, `client/TitanClientEvents.java` | PA-1 | A（此后只读） |
