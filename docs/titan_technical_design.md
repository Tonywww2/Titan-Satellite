...
| 环境系统 (Environment) | 低重力、极寒、缺氧、甲烷雨 | Forge 属性 (`ForgeMod.ENTITY_GRAVITY`) + Capability + 玩家 tick + 客户端维度特效 |
...
| `cryo_carapace` | 冰硅甲虫掉落，抗寒护甲强化材料 | 计划 |
...
- 生物效果 (`MobEffect`)：缺氧窒息、极寒冻伤等自定义效果（后续）。低重力优先用 Forge 重力属性而非自定义效果。
...
- `worldgen/noise_settings/titan.json`：骨架先复用/引用 `minecraft:overworld` 的 noise settings 快速跑通；后续替换为自定义（体现峡谷 + 沙丘 + 断崖的极端高差 0-320）。

### 3.2 群系 (Biomes) - 对应设计五大群系
`worldgen/biome/` 下每群系一个 JSON：

| biome id | 设计群系 | Y 参考 | 温度/降水基调 |
|---|---|---|---|
| `methane_abyss` | 液态甲烷深渊 | 0-64 | 极寒、无降水（甲烷海用流体填充） |
| `cratered_wastelands` | 撞击陨坑荒原 | 64-110 | 极寒、碎冰砾石 |
| `tholin_dune_sea` | 托林沙海 | 100-170 | 极寒、橙沙（占位已实现该 biome JSON） |
| `polar_labyrinth` | 极地迷宫冰原 | 160-240 | 极寒、冰迷宫 |
| `cryovolcanic_cliff` | 冰火山断崖 | 220-320 | 极寒、蓝冰垂直断崖 |

多群系分层用 `multi_noise` 的 `temperature/humidity/continentalness/erosion/depth/weirdness` 参数把群系压到不同 Y/地形区间（配合 noise_settings 的 `surface_rule` 铺设方块调色板）。
...
- 特征：甲烷湖、陨石坑、巨型沙脊、冰层天坑、发光晶体簇 -> `configured_feature` + `placed_feature`，挂到对应 biome 的 `features` 列表。
- 复杂/程序化地貌（如穿层的巨型陨石坑、天坑）可能需自定义 `Feature<C>` 类（`ForgeRegistries.FEATURES` 注册）。

---

## 四、环境系统 (Environment System)

土卫六的"极端体验"核心，无对应原版系统，需自建，但可复用 Forge 能力降低成本。

### 4.1 低重力 (Low Gravity ~14%)
- 首选：**Forge 重力属性** `ForgeMod.ENTITY_GRAVITY`。玩家进入维度时降低该属性（或每 tick 校正），配合永久 `Slow Falling` 观感。跳跃提升可用隐藏 `JumpBoost` 效果或自定义跳跃初速。
- 判定"是否在本维度"：`entity.level().dimension() == ModDimensions.TITAN_LEVEL`（`ResourceKey<Level>`）。

### 4.2 极寒 + 缺氧 (Cold + No Oxygen)
- 数据结构：给玩家挂 **Capability**（`AttachCapabilitiesEvent<Entity>`）存储 `oxygen`、`bodyHeat` 两个计量值。
- 逻辑：`PlayerTickEvent`（服务端）每 tick 更新——地表暴露且未穿"供氧/保暖装备"时按速率扣减，归零后周期性造成伤害（窒息/冻伤，可用自定义 `DamageType` 数据驱动）。
- 装备判定：检查护甲槽/饰品是否为模组的"保暖服 + 供氧罐"（后续里程碑加入这些物品）。
- 客户端 HUD：`RenderGuiOverlayEvent` 画氧气/体温条。

### 4.3 甲烷雨 (Liquid Methane Rain)
...
- 方块 `cryovolcanic_geyser`：`randomTick` 或 BlockEntity 周期切换"蓄力/喷发"状态；喷发时在 `Block#entityInside` / `stepOn` 给站立实体施加向上 `setDeltaMovement`（大垂直动能），配合缓降/滑翔快速跨越 Y 轴高差。
- 表现：喷发时生成冰晶/氨雾粒子 + 音效。
...
- M3 环境系统：重力/缺氧/极寒 Capability + HUD + 保暖供氧装备 + 伤害。
